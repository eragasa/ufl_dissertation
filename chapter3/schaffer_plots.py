...
def schaffer(x):
    ...
    return schaffer_f1(x_), schaffer_f2(x_)
...
objective_functions = (schaffer_f1,schaffer_f2)

def schaffer_cost_function(x,w1,w2):
    global n_simulations
    weights = (w1,w2)

    obj_functions = [schaffer_f1,schaffer_f2]
    C = 0
...
class ParetoProblem(object):
    ...
    @property
    def N_objective_functions(self):
        return len(self.objective_functions)

class SchafferParetoProblem(ParetoProblem):
    def __init__(self):
        ...
        self.objective_functions = (schaffer_f1,schaffer_f2)

        self.cost_function = schaffer_cost_function
...
def pareto_surface_from_conjugate_gradient_method():
    ...
    print("x0_N = {}".format(len(weights)))
    print("w_N = {}".format(len(weights)))
    return df
...
if True:
    ...
    plt.tight_layout()
    fig.savefig("kurasaw_cg.jpg",dpi=1200)
    plt.show()
...
if True:
    ...
    ax.set_ylabel(r"$f_2(x)$")
    plt.tight_layout()
    fig.savefig("schaffer_mc.jpg",dpi=1200)
    plt.show()
...
if True:
    ...
    ax.set_ylabel(r"$f_2(x)$")
    plt.tight_layout()
    fig.savefig("schaffer_kde.jpg",dpi=1200)
    plt.show()
    print(n_simulations)
...
if False:
    ...
    ax[i].set_xlabel(r"$f_1(x)$")
    ax[i].set_ylabel(r"$f_2(x)$")
    ax[i].set(aspect='equal')
    plt.tight_layout()
    fig.savefig("comparison.jpg",dpi=1200)
    plt.show()
...
if True:
    ...
    ax.plot(
        ...
        #mc_df.loc[mc_pareto_rows,'f1'],
        #mc_df.loc[mc_pareto_rows,'f2'],
        c='blue',linewidth=1,marker=".",ms=5,label="CG",alpha=.7
    )
    ...
    ax.plot(
        ...
        #mc_df.loc[mc_pareto_rows,'f1'],
        #mc_df.loc[mc_pareto_rows,'f2'],
        c='green',linewidth=1,marker=".",ms=5,label= "MC",alpha=.7
    )
    ax.plot(
        kde_pareto_df['f1'],
        kde_pareto_df['f2'],
        #mc_df.loc[mc_pareto_rows,'f1'],
        #mc_df.loc[mc_pareto_rows,'f2'],
        c='red',linewidth=1,marker=".",ms=5,label="KDE",alpha=.7
    )
    ax.set_xlabel(r"$f_1(x)$")
    ax.set_ylabel(r"$f_2(x)$")
    ax.legend(loc="upper right")
    plt.tight_layout()
    fig.savefig("schaffer_comparison.jpg",dpi=1200)
    plt.show()
