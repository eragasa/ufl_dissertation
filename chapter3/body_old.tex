...
The previous chapter review the different computational tools associated with atomistic simulations.  Within this  chapter, we discuss the current state of potential development within the context of parameterization.  This chapter frames potential development in a broad mathematical context so that the current issues with optimization can be explained from explained using the language of optimization theory.  Additionally, the larger scope of this dissertation is to introduce new tools and a techniques to the field of potential development.

This chapter provides the motivation for an automated framework for potential development by framing an empirical interatomic potential (EIP) as a surrogate model for the potential surface, which maps the atomic configuration onto energies.  Then starting with the concept of curve fitting, the alternative method of parameterization of force matching\cite{ercolessi1994_fitting_forcematching} is briefly discussed before a discussion of the selection of a fitting database, which is used in parameterization.  The problem of potential parameterization can then be discussed as a multi-objective optimization problem, which is typically solved by minimizing the weighted some of squares between predicted and reference material properties.

At this point, the discussion will then turn to problems with optimization within the scalar optimization context.

\section{Potential Energy Surfaces}

The potential energy surface (PES)is a function which he energy of a collection of atoms as a function of the positions of its nuclei, $\{\bm{R}\}$ onto energies, $V:\{\bm{R}\}$.  Using the analogy of a landscape, the potential energy surface (PES) represents a mapping of atomic positions to energies: $V:\bm of a material system to an energies.
